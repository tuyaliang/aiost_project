...
/*
 * Note: We know the request doesn't have and make the hole, because
 * we checked this is contiguous in fat_direct_IO().
 */
static int fat_direct_IO_get_block(struct inode *inode, sector_t iblock,
				   struct buffer_head *bh_result, int create)
{
	struct super_block *sb = inode->i_sb;
	...
	last_block = i->mmu_private >> sb->s_blocksize_bits;
	if (last_block < (iblock + max_blocks)) {
		if (i->mmu_private & (sb->s_blocksize - 1)) {
			i->mmu_private |= (sb->s_blocksize - 1);
			i->mmu_private++;
		}
	}
			     ...
			     loff_t offset)
{
	...
	get_block_t *get_block = fat_get_block;
	ssize_t ret;
	...
	if (iov_iter_rw(iter) == WRITE) {
		 ...
		 * But we must fill the all of remaining area or hole
		 * by nul for updating ->mmu_private.
		 *
		 * If this request fills, we can update ->mmu_private
		 * by fat_direct_io_get_block().
		 *
		 * Otherwise, return 0, and fallback to normal buffered write.
		 */
		if (MSDOS_I(inode)->mmu_private < offset)
			return 0;

		get_block = fat_direct_IO_get_block;
	}
	 ...
	 */
	ret = blockdev_direct_IO(iocb, inode, iter, offset, get_block);
