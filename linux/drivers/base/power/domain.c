...
static int pm_genpd_restore_early(struct device *dev);
#endif
...
#ifdef CONFIG_PM_WARP
	if (pm_device_down)
		return  pm_genpd_restore_early(dev);
#endif
...
/**
 * pm_genpd_restore_early - Early restore of device in an I/O PM domain.
...
static int pm_genpd_restore_early(struct device *dev)
{
	...
	return genpd->suspend_power_off ? 0 : pm_generic_restore_early(dev);
}
