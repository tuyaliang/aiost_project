...
//#ifdef CONFIG_EHCI_TEST_REGISTERS
	       ...
	       char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	struct ehci_hcd	*ehci = hcd_to_ehci(bus_to_hcd(bus));
	unsigned long	flags;
	char		buf[32];
	size_t		size;
	...
	size = snprintf(buf, 31, "0x%x 0x%x\n",
			test_reg_offset,
			ehci_readl(ehci, (void *)ehci->regs + test_reg_offset));
	spin_unlock_irqrestore (&ehci->lock, flags);
	       ...
	       const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	spin_lock_irqsave (&ehci->lock, flags);
	ehci_writel(ehci, (u32)val, (void *)ehci->regs + offs);
	spin_unlock_irqrestore (&ehci->lock, flags);
...
static inline void create_debug_files (struct ehci_hcd *ehci)
{
	...
	if (!debugfs_create_file("test_regs", 0600, ehci->debug_dir, bus,
