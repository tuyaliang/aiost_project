	...
	enum led_brightness brightness)
{
	...
	if (host->quirks2 & SDHCI_QUIRK2_LED_ABUSED_AS_CARD_POWER)
		return;
...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if (host->mmc->flags &  MMC_UHS2_SUPPORT) {
		sdhci_writew(host, SDHCI_UHS2_MAKE_BLKSZ(SDHCI_DEFAULT_BOUNDARY_ARG,
			data->blksz), SDHCI_UHS2_BLOCK_SIZE);
...
static void sdhci_do_set_ios(struct sdhci_host *host, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_OFF) {
		if (host->quirks2 & SDHCI_QUIRK2_LED_ABUSED_AS_CARD_POWER)
			sdhci_deactivate_led(host);
		else if (host->quirks2 & SDHCI_QUIRK2_OPS_SD_POWER_CONTROL)
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->data) {
		...
		/* While tunig from mmc_send_tuning(), CMD ERROR ->
		 * DATA ERROR might happen. CMD ERROR call
		 * tasklet_schedule() in sdhci_cmd_irq() and set
		 * host->data = NULL. So ignore this error message
		 * while tuning.
		*/
		command = SDHCI_GET_CMD(sdhci_readw(host, SDHCI_COMMAND));
		if (!(command == MMC_SEND_TUNING_BLOCK ||
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (host->ops->set_version_quirk)
		host->version = host->ops->set_version_quirk(host);
