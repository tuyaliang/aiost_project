...
static int exfat_ioctl_set_attributes(struct file *file, u32 __user *user_attr)
{
	...
	attr |= (EXFAT_I(inode)->fid.attr & ATTR_VOLUME) |
...
static int exfat_ioctl_volume_id(struct inode *inode, u32 __user *user_attr)
{
	struct exfat_sb_info *sbi = EXFAT_SB(inode->i_sb);
...
#endif
{
	...
	case EXFAT_IOCTL_GET_VOLUME_ID:
		return exfat_ioctl_volume_id(inode, user_attr);
#if EXFAT_CONFIG_KERNEL_DEBUG
...
static ssize_t
exfat_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
...
static int exfat_direct_IO_get_block(struct inode *inode, sector_t iblock,
				   struct buffer_head *bh_result, int create)
{
	struct super_block *sb = inode->i_sb;
	...
	last_block = ei->mmu_private >> sb->s_blocksize_bits;
	if (last_block < (iblock + max_blocks)) {
		if (ei->mmu_private & (sb->s_blocksize - 1)) {
			ei->mmu_private |= (sb->s_blocksize - 1);
			ei->mmu_private++;
		}
	}
	return exfat_get_block(inode, iblock, bh_result, create);
}
			   ...
			   loff_t offset)
{
	...
	size_t count = iov_iter_count(iov_iter);
	get_block_t *get_block = exfat_get_block;
	ssize_t ret;

	if (iov_iter_rw(iov_iter) == WRITE) {
		if (EXFAT_I(inode)->mmu_private < offset + count)
			return 0;
		get_block = exfat_direct_IO_get_block;
	}

	ret = blockdev_direct_IO(iocb, inode, iov_iter, offset, get_block);
	if ((ret < 0) && (iov_iter_rw(iov_iter) == WRITE))
